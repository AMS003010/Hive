    ...
    return(
        <div className="board-layout p-8 h-[31rem] mb-8 border border-gray-200 mx-4 rounded-3xl overflow-hidden shadow-xl overflow-y-auto">
            ...
            <div className="flex flex-col">
                    {users ? (
                        users.length!=0 ? (
                            users.map((user_obj: User) => {
                                const color = gradColor()
                                return (
                                    <div key={user_obj.id} className="flex justify-between items-center gap-4 bg-white cursor-pointer rounded-2xl p-4 mt-4 shadow-md transform transition-transform hover:-translate-y-2 hover:shadow-lg">
                                        <div className="flex justify-between items-center gap-4">
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <div className="text-2xl font-light">{user_obj.name}</div>
                                                <div className="flex justify-start items-center gap-2">
                                                    <span className='p-1.5 text-xs font-medium text-blue-80 bg-blue-200 rounded-lg bg-opacity-50'>{user_obj.email}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="border rounded-lg p-2">
                                            <select id="options" className="p-2">
